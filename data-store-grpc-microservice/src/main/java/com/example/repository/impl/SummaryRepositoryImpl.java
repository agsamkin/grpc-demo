    ...
    public Optional<Summary> findBySensorId(long sensorId, Set<Data.MeasurementType> measurementTypes, Set<Summary.SummaryType> summaryTypes) {
        ...
        try (Jedis jedis = jedisPool.getResource()) {
            ...
            if (measurementTypes.isEmpty() && !summaryTypes.isEmpty()) {
                return getSummary(sensorId, Set.of(Data.MeasurementType.values()), summaryTypes, jedis);
            } else if (!measurementTypes.isEmpty() && summaryTypes.isEmpty()) {
                return getSummary(sensorId, measurementTypes, Set.of(Summary.SummaryType.values()), jedis);
            } else return getSummary(sensorId, measurementTypes, summaryTypes, jedis);
        }
